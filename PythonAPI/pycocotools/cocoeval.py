...
class COCOeval:
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        #         g['_ignore'] = 0
        for g in gt:
            if g['ignore'] or (g['rel_size']<aRng[0] or g['rel_size']>aRng[1]):
                g['_ignore'] = 1
            else:
                g['_ignore'] = 0
        ...
        a = np.array([d['rel_size']<aRng[0] or d['rel_size']>aRng[1] for d in dt]).reshape((1, len(dt)))
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            if len(self.params.maxDets)==1:
                ...
                stats[7] = _summarize(1, areaRng='medium', maxDets=self.params.maxDets[-1],  iouThr=self.params.iouThr)
                stats[8] = _summarize(1, areaRng='large', maxDets=self.params.maxDets[-1],  iouThr=self.params.iouThr)
                stats[9] = _summarize(0, maxDets=self.params.maxDets[0],  iouThr=self.params.iouThr)
                stats[10] = _summarize(0, maxDets=self.params.maxDets[1],  iouThr=self.params.iouThr)
                stats[11] = _summarize(0, maxDets=self.params.maxDets[-1],  iouThr=self.params.iouThr)
                stats[12] = _summarize(0, areaRng='micro', maxDets=self.params.maxDets[-1],  iouThr=self.params.iouThr)
                stats[13] = _summarize(0, areaRng='v-tiny', maxDets=self.params.maxDets[-1],  iouThr=self.params.iouThr)
                stats[14] = _summarize(0, areaRng='tiny', maxDets=self.params.maxDets[-1],  iouThr=self.params.iouThr)
                stats[15] = _summarize(0, areaRng='small', maxDets=self.params.maxDets[-1],  iouThr=self.params.iouThr)
                stats[16] = _summarize(0, areaRng='medium', maxDets=self.params.maxDets[-1],  iouThr=self.params.iouThr)
                stats[17] = _summarize(0, areaRng='large', maxDets=self.params.maxDets[-1],  iouThr=self.params.iouThr)
                return stats
            else:
...
class Params:
    ...
    def setDetParams(self, maxDets=[1, 10, 100], iouThr=None):
        ...
        self.areaRng = [[0, 100], [0, 0.38096593373484317], [0.38096593373484317, 1.5238637349393727],  
                        [1.5238637349393727, 3.0477274698787453], [3.0477274698787453, 6.095454939757491], 
                        [6.095454939757491, 18.28636481927247], [18.28636481927247, 100]]
        self.areaRngLbl = ['all', 'micro', 'v-tiny', 'tiny', 'small', 'medium', 'large']
        self.useCats = 1
